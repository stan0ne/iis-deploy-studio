...
- **GitHub Actions CI** (`.github/workflows/ci.yml`) — build + test on push/PR to main/master/develop. `windows-latest`, .NET 10, warnings-as-errors, xUnit TRX + code coverage artifacts.
- **GitHub Actions release** (`.github/workflows/release.yml`) — `v*` tag or manual dispatch → publish self-contained + WiX MSI + GitHub Release with MSI artifact (90-day retention). Includes build manifest generation and MSI size validation.
- **`docs/INSTALL.md`** — end-user installation guide: system requirements (Server 2012+), interactive/silent/PowerShell install, uninstall, upgrade, plugin folder, logs/diagnostics, troubleshooting, enterprise deployment notes.
- **`PowerShellExecutionService` ExecutionPolicy bypass** — `InitialSessionState.CreateDefault()` with `ExecutionPolicy.RemoteSigned` overrides the system's `Restricted` policy at the runspace level, enabling `Import-Module ServerManager` and `Get-WindowsFeature` to load without modifying the machine-wide setting.
- **`Import-Module ServerManager`** — added to `IisFeatureScanner.CheckIisFeaturesViaPowerShellAsync` script so `Get-WindowsFeature` can resolve its module dependency.
...
- **Multi-resolution application icon** — new `src/UI/app.ico` (9,801 bytes) packaged with 7 PNG-encoded slots at 16/24/32/48/64/128/256, generated by `scripts/generate-icon.cs` (.NET 10 file-based program using `System.Drawing.Common`). The design is a rounded box with vertical gradient (`#4CC2FF` → `#2196F3`) and a darker lid (`#1565C0`) plus a centered white upward arrow — directly referencing the `.iispackage` deploy flow and the app's accent color. `IISDeploy.UI.csproj` already references it via `<ApplicationIcon>app.ico</ApplicationIcon>`, so the single-file publish automatically embeds every slot.
...
### Changed
- **Toolbar icon redesign** — all 6 toolbar icons (Refresh, Export, Import, Scan, Logs, Reports) now use `DynamicResource` brushes (`TextSecondaryBrush`, `AccentBrush`, `SuccessBrush`, `WarningBrush`) instead of hardcoded hex colors. Every icon is theme-aware and visible in both light and dark modes.
- **Server info card icons** — replaced emoji characters (🖥, 🪟, 🕒) with Unicode text characters (⬡, ▣, ◷) that respect the WPF `Foreground` brush, ensuring consistent rendering across themes. Each card icon uses a distinct `DynamicResource` brush (`PrimaryBrush`, `AccentBrush`, `SuccessBrush`).
- **Center placeholder icon** — replaced hardcoded `LinearGradientBrush` (#4F8EF7→#7C3AED) with `{DynamicResource PrimaryBrush}` for theme consistency.
- `README.md` updated with MSI build step and link to `docs/INSTALL.md`.
...
### Fixed
- **Scan button PowerShell failure** — `Get-WindowsFeature` failed with "module could not be loaded" when system ExecutionPolicy was `Restricted`. Fixed by setting `ExecutionPolicy.RemoteSigned` on the PowerShell runspace and adding `Import-Module ServerManager` to the scan script.
- **Dark mode icon visibility** — toolbar icons without explicit `Foreground` used the default black TextBlock color, which was invisible on dark backgrounds. All icons now use theme-aware DynamicResource brushes.
...
- `dotnet publish -c Release -r win-x64 --self-contained true` ✅ — `release/IISDeployStudio.exe` (~174 MB)
- `dotnet build installer/IISDeployStudio.Installer.csproj -c Release` ✅ — `installer/bin/Release/IISDeployStudio-Setup.msi` (~58 MB)
- `pwsh -File .\scripts\validate-release.ps1` ✅ — 8/8 gates PASS
